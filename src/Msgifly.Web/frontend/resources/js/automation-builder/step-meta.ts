...
// A flat pastel hex (e.g. "#eff6ff") tuned to sit on a WHITE canvas reads as a jarring, mismatched
// solid-white blob once the page is in dark mode, since it doesn't adapt to whatever's behind it.
// A translucent rgba of the same accent blends into either theme's actual background instead of
// assuming one.
...
const STEP_ACCENTS: Record<StepType, string> = {
  SendMessage: '#2563eb',
  SendTemplate: '#7c3aed',
  SendButtons: '#0d9488',
  Condition: '#ea580c',
  Wait: '#d97706',
  UpdateContactField: '#db2777',
  SendWebhook: '#0891b2',
  Stop: '#dc2626',
};

export const STEP_COLOR: Record<StepType, StepColor> = Object.fromEntries(
  Object.entries(STEP_ACCENTS).map(([type, accent]) => [type, { accent, tint: tintOf(accent) }])
) as Record<StepType, StepColor>;
